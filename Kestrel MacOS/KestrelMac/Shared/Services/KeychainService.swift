...
struct KeychainService {
    ...
    static func saveVNCPassword(_ password: String, for serverID: UUID) throws {
        try saveGenericPassword(password, service: vncPasswordService, account: serverID)
    }

    static func loadVNCPassword(for serverID: UUID) throws -> String {
        try loadGenericPassword(service: vncPasswordService, account: serverID)
    }

    static func deleteVNCPassword(for serverID: UUID) throws {
        try deleteGenericPassword(service: vncPasswordService, account: serverID)
    }

    // MARK: - RDP Passwords

    static func saveRDPPassword(_ password: String, for serverID: UUID) throws {
        try saveGenericPassword(password, service: rdpPasswordService, account: serverID)
    }

    static func loadRDPPassword(for serverID: UUID) throws -> String {
        try loadGenericPassword(service: rdpPasswordService, account: serverID)
    }

    static func deleteRDPPassword(for serverID: UUID) throws {
        try deleteGenericPassword(service: rdpPasswordService, account: serverID)
    }

    // MARK: - Generic Helpers

    private static func saveGenericPassword(_ password: String, service: String, account: UUID) throws {
    ...
    private static func loadGenericPassword(service: String, account: UUID) throws -> String {
        let query: [String: Any] = [
            kSecClass as String: kSecClassGenericPassword,
            kSecAttrService as String: service,
            kSecAttrAccount as String: account.uuidString,
        ...
        let status = SecItemCopyMatching(query as CFDictionary, &result)
        guard status == errSecSuccess, let data = result as? Data,
              let password = String(data: data, encoding: .utf8) else { throw KeychainError.loadFailed }
        return password
    }

    private static func deleteGenericPassword(service: String, account: UUID) throws {
        let query: [String: Any] = [
            kSecClass as String: kSecClassGenericPassword,
            kSecAttrService as String: service,
            kSecAttrAccount as String: account.uuidString
